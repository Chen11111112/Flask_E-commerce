...
const BASE_URL = 'http://127.0.0.1:5000/practice/api/products'; // 這裡記得改IP或Local !!
...
function renderError() {
    if (error) {
        ...
        $errorMsg.style.backgroundColor = "#F443369C";
        $errorMsg.style.color = "#000000F6";
    } else {
...
function renderShoppingCart() {
    ...
    if (cartItems.length === 0) {
        ...
        $btnCheckout.disabled = false; // 不可點擊
        return;
    }
...
function renderLoading() {
    if (loading) {
        $errorMsg.style.display = 'block';
        $errorMsg.textContent = "請燒等....";
        $errorMsg.style.backgroundColor = "#008BBA3F";
        $errorMsg.style.color = "#000000F6";
    } else {
        $errorMsg.style.display = 'none';
    }
...
async function handleCheckoutSubmit() {
    ...
    try {
        ...
        if (!response.ok) {
            // 如果後端有傳回錯誤訊息（如：庫存不足），就顯示它
            // 否則顯示預設的錯誤提示
            const errorMessage = result.message || "伺服器發生未知錯誤";
            throw new Error(errorMessage);
        }
        
        // 3. 處理成功
        alert(`結帳成功！訂單編號：${result.orderNo}，請檢查信箱。`);
        cartItems = [];
        error = null;
        renderAll();
        fetchProducts();
